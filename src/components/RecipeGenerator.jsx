...
const RecipeGenerator = () => {
  ...
  const handleClick = () => {
    startTransition(async () => {
      ...
      try {
        ...
        console.log(error);
        setError('レシピの生成に失敗しました。しばらく経ってから再度お試しください');
      }
    })
  }
  ...
  return (
    <div className="text-center mt-8 ">
      <button className="btn" onClick={handleClick} disabled={isPending}>{isPending ? <Spinner /> : 'レシピ生成'} </button>
      {error && <p className="text-red-500">{error}</p>}
      { recipe && (
        <div className="border rounded-2xl shadow-md p-6 my-8">
          <div className="flex justify-between items-start mb-4">
            <div>
              <h2 className="text-start text-xl font-semibold">{recipe.name}</h2>
              ...
              <ul className="grid grid-cols-3 gap-2">
                {recipe.ingredients.map((ingredient, index) => (
                  <li key={index} className="flex items-center">
                    <span className="w-4 h-4 rounded-full bg-green-100 text-xs flex items-center justify-center mr-2">・</span>
